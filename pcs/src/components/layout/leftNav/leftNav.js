...
import ContosoIcon from '../../../assets/icons/JCM-TECH.svg';
...
class LeftNav extends Component {
  ...
  constructor(props) {
    ...
    this.tabLinks = tabConfig.map((item, index) =>
      <Link key={index} to={item.path} className="leftnav-item-container noHamburguer" activeClassName="active">
  ...
  render() {
    return (
      <div className={`left-nav ${this.state.isExpanded ? 'expanded' : ''}`}>
        ...
        <div className="leftnav-item-container title">
          <div className="leftnav-item-icon">
            <img src={ContosoIcon} className="page-title-icon" alt="JCM-TECH" />
          </div>
